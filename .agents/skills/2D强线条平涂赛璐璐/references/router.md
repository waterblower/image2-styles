...
route_rules:
  - For face requests, read face plus only visible materials such as skin, hair, makeup, metal, fabric, or leather.
  - For full-body requests, read full_body plus all visible clothing and body materials.
  - For environments without people, do not read face or full_body.
  - For objects, read object plus only the object's materials.
  - For mixed scenes, read only the actually present subject and material files.
```
